...
1. Encapsulate the `Loan` and `ConventionalLoan` interfaces into a namespace called `Loans`.

1. Update the `Loan` and `ConventionalLoan` interfaces so they are visible outside of the `Loans` namespace.

1. Encapsulate the three functions into a namespace called `LoanPrograms`.

1. In the `interestOnlyLoan` and `conventionalLoan` functions:

   1. Update the references to the `Loan` and `ConventionalLoan` interfaces so the functions can implement them.

   1. Make the functions visible outside of the `LoanPrograms` namespace.

1. Update function calls in the `loanA` and `loanB` variable declarations.
...
1. Create two new TypeScript files in your workspace, **module07_loans.ts** and **module07_loan-programs.ts**.

1. Move the `Loans` namespace from **module07_main.ts** to **module07_loans.ts**.

1. Move the `LoanPrograms` namespace from **module07_main.ts** to **module07_loan-programs.ts**.

1. Add the required `reference` statements to the **module07_loan-programs.ts** and **module07_main.ts** files.

1. At the command prompt, run the `tsc` command to compile all the `.ts` files, and then run the `tsc --outFile main.js module07_main.ts` command to create a single JavaScript file named **main.js**.
...
1. Create a new folder called **Namespace** in your project folder.

1. Copy **module07_main.ts**, **module07_loans.ts,** and **module07_loan-programs.ts** from the root folder and paste them into the **Namespaces** folder. This retains a copy of your work from Exercise 2.

1. Return to the root folder.

1. In **module07_loans.ts**, remove the `namespace` declaration and retain the `export` keyword on the interface declaration.

1. In **module07_loan-programs.ts**:

   1. Remove the `namespace` declaration.

   1. Remove the `reference` statement and replace it with an ``import`` statement that imports the `Loan` and `ConventionalLoan` interfaces from **module07_loans.ts**. For convenience, you can import both interfaces using one import statement and assign them to a variable called `Loans`.

   1. Retain the `export` keyword on the two function declarations.

1. In **module07_main.ts**, replace the two `reference` statements with one `import` statement that imports the `interestOnlyLoan` and `conventionalLoan` functions from **module07_loan-programs.ts**. For convenience, assign the functions to a variable called `LoanPrograms`.

1. At the command prompt, run the `tsc` command to compile **module07_main.ts** using the `--module commonjs` option.

1. Test your work by running the **module07_main.js** file.
